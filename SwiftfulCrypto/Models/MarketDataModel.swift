...
//  SwiftfulCrypto
//
//  Created by Alvaro Ordonez on 8/10/25.
//
...
struct MarketDataModel: Codable {
    //removed the lines below because it won't be used in this app
    /*
    let activeCryptocurrencies, upcomingIcos, ongoingIcos, endedIcos: Int?
    let markets: Int?
     */
    //removed optional character '?' because we want to make sure we get a value back(don't want to get back a nil)
    ...
    /*
     created CodingKeys because the JSON Data above has, for example, the property total_market_cap, which is not camel cased. It is snake case. Need to create a camel case equivalent for the snake cased properties
     */
    ...
    //computed property
    //if a key with "usd" is found, a String value for that key is returned
    var marketCap: String {
        //totalMarketCap is a dictionary with a key:value of String:Double
        //totalMarketCap: [String:Double]
        //if a key with "usd" is found, return true, then return the value of that
        //item as a String
        if let item = totalMarketCap.first(where: { (key, value) -> Bool in
            return key == "usd"
        }) {
            //found an item with a key of "usd", return the value
            //convert the Double to a String
            //item.value is of type Double... call the computed property
            //formattedWithAbbreviations() within Double extension
            return "$" + item.value.formattedWithAbbreviations()
            
            //return "\(item.value)"
        }
        
        //equivalent way of writing above code
        /*
